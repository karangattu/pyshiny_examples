...
app_ui = ui.page_fillable(
    ui.layout_sidebar(
        ui.sidebar(
            ...
            ui.input_text(
                id="new_label", label="Enter new label", value="Updated Label"
            ),
            ui.input_checkbox(
                id="toggle_disabled", label="Disable button", value=False
            ),
            # Button to trigger the update
            ui.input_action_button(
                id="update_btn", label="Update Button", class_="mt-3"
            ),
        ),
        # Main panel content
        ui.card(
